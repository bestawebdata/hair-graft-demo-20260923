(() => {
  ...
  const content = {
    about: () => `<p class="content-eyebrow">ABOUT THE TREATMENT</p><h2 tabindex="-1">自分の組織を、<br>頭皮の治療に用いる。</h2><p>${e(C.medical.overview)}</p><p>${e(C.medical.purpose)}</p><figure class="info-image">${sourceImage(C.treatmentVisuals.equipment)}<figcaption>${e(C.treatmentVisuals.equipment.name)}｜${e(C.treatmentVisuals.equipment.source)}</figcaption></figure><p class="source-note">${e(C.treatmentVisuals.note)}</p><h3>治療の流れ</h3><ol class="treatment-steps">${C.medical.steps.map((s, i) => `<li><b>0${i + 1}</b><div><strong>${e(s.title)}</strong><p>${e(s.text)}</p></div></li>`).join("")}</ol>${action("リスク・副作用を確認する", "price", "outline")}${action("希望地域を選ぶ", "regions")}`,
    doctor: () => `<p class="content-eyebrow">DOCTOR</p><div class="doctor-summary"><img src="../assets/doctor/ueda-portrait.webp" width="1000" height="1500" alt="医師紹介資料に掲載された上田敬博医師"><div><h2 tabindex="-1">${e(C.doctor.name)}</h2><p>${e(C.doctor.credential)}</p></div></div><p>熱傷治療と、自家培養表皮を用いた治療に携わってきた医師として、提供資料に紹介されています。</p><h3>主な経歴</h3><ol class="career-list">${C.doctor.career.map(([year, detail]) => `<li><time>${e(year)}</time><span>${e(detail)}</span></li>`).join("")}</ol><p class="source-note">${e(C.doctor.source)}</p><p class="source-note">担当医・監修体制は調整中です。すべての施術を上田医師が担当することを示すものではありません。</p>${action("治療について見る", "about", "outline")}${action("施術希望を登録する", "register")}`,
    cases: () => {
      const B = C.brochureGallery;
      const records = B?.enabled && B.publicationAuthorized ? B.records : [];
      return `<p class="content-eyebrow">CASE FOLLOW-UP</p><h2 tabindex="-1">経過を、丁寧に見ていく。</h2><p>${e(records.length ? B.intro : "症例写真は掲載準備中です。")}</p><div class="line-cases">${records.map((r) => `<article class="case-card"><h3>${e(r.title)}</h3><div class="case-pair">${r.observations.map((o) => `<figure><figcaption>${e(o.label)}</figcaption>${sourceImage(o)}</figure>`).join("")}</div></article>`).join("")}</div>${records.length ? `<p class="source-note">出典：${e(B.source)}</p><p>${e(B.note)}</p><p class="source-note">${e(B.details)}</p>` : ""}${action("料金・リスクを確認する", "price", "outline")}${action("施術希望を登録する", "register")}`;
    },
    price: () => `<p class="content-eyebrow">PRICE & RISKS</p><h2 tabindex="-1">費用とリスクについて</h2><div class="cost-card"><p>${e(C.treatment.name)}</p><div class="cost-amount"><b>${e(C.treatment.price)}</b><span>${e(C.treatment.priceUnit)}</span><small>（仮）</small></div><p class="cost-state">${e(C.treatment.priceStatus)}。正式な金額ではありません。</p><p>自由診療・保険適用外</p><p class="small muted">税込・税別、施術範囲・回数、診察・麻酔・再診等の費用が含まれる範囲は確認中です。</p></div><p class="source-note">同意書では再施術は別途費用、返金保証なしとされています。最終条件は正式なご案内でご確認ください。</p><h3>主なリスク・副作用</h3><ul class="risk-list">${C.medical.risks.map((r) => `<li>${e(r)}</li>`).join("")}</ul><h3>ダウンタイム・施術後の生活</h3><dl class="detail-list">${C.medical.downtime.map(([k, v]) => `<div><dt>${e(k)}</dt><dd>${e(v)}</dd></div>`).join("")}</dl><h3>診察時にご確認ください</h3><p>${e(C.medical.precautions)}</p><p class="source-note">治療説明資料・施術同意書に基づく説明です。実際には担当医の指示に従ってください。異常な痛み・腫れ・発熱などがある場合は、速やかに医療機関へ連絡してください。</p>${action("施術希望を登録する", "register")}`,
