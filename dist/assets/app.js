(() => {
  ...
  const equipmentPanel = `<div class="wrap equipment-feature" aria-labelledby="equipment-title">
    <figure class="equipment-main">${sourceImage(V.equipment)}<figcaption><span>皮膚組織を微細化する機器</span>${escape(V.equipment.name)}</figcaption></figure>
    <div class="equipment-copy"><p class="eyebrow">EQUIPMENT & PROCESS</p><h3 id="equipment-title">治療に用いる機器を、<br>写真で知る。</h3><p>専用カートリッジとリジェネラ装置を用いて、採取した皮膚組織から注入液を作成します。</p>
      <figure class="cartridge-detail">${sourceImage(V.cartridge)}<figcaption><b>${escape(V.cartridge.caption)}</b></figcaption></figure>
    </div><p class="equipment-note">${escape(V.note)}</p></div>`;
  const procedureGallery = `<div class="procedure-gallery-heading"><p class="eyebrow">PROCEDURE IN PICTURES</p><h3>写真で見る、治療の流れ。</h3><p>採取した皮膚組織の処理から、気になる頭皮への注入まで。</p></div><div class="procedure-gallery">${V.procedurePhotos.map((photo) => `<figure class="procedure-photo"><div class="procedure-photo-frame">${sourceImage(photo)}<span>治療の工程</span></div><figcaption><h4>${escape(photo.label)}</h4><p>${escape(photo.caption)}</p></figcaption></figure>`).join("")}</div>`;
  const treatmentHighlights = `<div class="wrap treatment-highlights"><div class="highlights-heading"><p class="eyebrow">TREATMENT AT A GLANCE</p><h3>施術から、その後の生活まで。</h3></div><div class="highlight-grid">${C.medical.highlights.map((item, i) => `<article class="highlight-card"><span aria-hidden="true">0${i + 1}</span><h4>${escape(item.title)}</h4><p>${escape(item.text)}</p></article>`).join("")}</div><p class="highlights-aftercare">${escape(C.medical.aftercareNote)}</p></div>`;
  document.getElementById("page-details").innerHTML = `
    <section id="about" class="section about"><div class="wrap two-column">
      <div class="section-heading"><p class="eyebrow">ABOUT THE TREATMENT</p><h2>自分の皮膚組織で、<br>髪の太さと密度にアプローチ。</h2><span class="section-number" aria-hidden="true">01</span></div>
      <div class="about-body"><p class="lead">${escape(C.medical.purpose)}</p><div class="treatment-diagram" aria-label="治療の概要：皮膚組織の採取、微細化、頭皮へ注入"><span><b>採取</b><small>耳の後ろの皮膚組織</small></span><i aria-hidden="true">→</i><span><b>微細化</b><small>専用機器で注入液に</small></span><i aria-hidden="true">→</i><span><b>注入</b><small>薄毛が気になる頭皮</small></span></div><p>${escape(C.medical.overview)}</p><p class="source-note">${escape(C.medical.purposeNote)}</p><a class="text-link" href="#risks">リスクとダウンタイムを確認する</a></div>
    </div>${treatmentHighlights}${equipmentPanel}</section>
    <section id="doctor" class="section wrap"><div class="doctor-panel"><div class="doctor-intro"><p class="eyebrow">DOCTOR</p><p class="doctor-kicker">${escape(C.doctor.headline)}</p><h2>${escape(C.doctor.name)}<small>${escape(C.doctor.credential)}</small></h2><p class="doctor-english">${escape(C.doctor.english)}</p><p class="doctor-description">${escape(C.doctor.introduction)}</p><p class="doctor-footnote">${escape(C.doctor.availability)}</p></div><div class="doctor-career"><h3>主な経歴</h3><ol>${C.doctor.career.map(([year, text]) => `<li><time>${escape(year)}</time><p>${escape(text)}</p></li>`).join("")}</ol></div></div><div class="doctor-gallery">${C.doctor.gallery.map((photo) => `<figure class="doctor-photo ${escape(photo.className)}"><div class="doctor-photo-frame"><img src="${escape(photo.image)}" width="${photo.width}" height="${photo.height}" alt="${escape(photo.alt)}" loading="lazy"></div><figcaption><span>${escape(photo.label)}</span><p>${escape(photo.caption)}</p></figcaption></figure>`).join("")}</div></section>
    <section id="for-you" class="section for-you"><div class="wrap"><div class="section-heading"><p class="eyebrow">FOR YOU</p><h2>こんなお悩みを、<br class="mobile-only">まずはご相談ください。</h2></div><div class="concerns"><div><span aria-hidden="true">01</span><h3>以前より髪が<br>細くなったと感じる</h3></div><div><span aria-hidden="true">02</span><h3>内服薬を使うことが<br>難しく、相談したい</h3></div><div><span aria-hidden="true">03</span><h3>自分の皮膚組織を用いる<br>治療について知りたい</h3></div></div><p class="muted small">治療が適しているか、頭皮の状態やこれまでの治療経験を踏まえて医師が判断します。</p></div></section>
    <section id="flow" class="section wrap"><div class="section-heading"><p class="eyebrow">TREATMENT FLOW</p><h2>納得して進めるための、<br>治療の流れ。</h2></div><p class="section-intro">希望登録後、開催条件が整った地域から日程をご案内する予定です。施術は、診察・説明と同意の後に行います。</p>${procedureGallery}<ol class="flow-list">${C.medical.steps.map((step, i) => `<li><span class="step-index">0${i + 1}</span><div><h3>${escape(step.title)}</h3><p>${escape(step.text)}</p></div></li>`).join("")}</ol><div class="subtle-note"><strong>施術時間の目安</strong><span>${escape(C.medical.duration)}。診察・待ち時間を含む所要時間は、日程のご案内時にお知らせします。</span></div></section>
    ...
    <section id="price" class="section wrap"><div class="two-column price-layout"><div class="section-heading"><p class="eyebrow">PRICE</p><h2>費用について</h2><p class="section-intro">自由診療のため、<br>健康保険は適用されません。</p></div><div class="price-card"><div class="price-top"><h3>${escape(C.treatment.name)}</h3>${badge(C.treatment.priceStatus)}</div><p class="price-amount"><strong>${escape(C.treatment.price)}</strong><span>${escape(C.treatment.priceUnit)}</span><small>（仮）</small></p><p class="price-status">料金は検討中です。正式な金額ではありません。</p><dl class="price-details"><div><dt>税込・税別の区分</dt><dd>確認中</dd></div><div><dt>施術範囲・回数</dt><dd>確認中</dd></div><div><dt>診察・麻酔・再診等の費用</dt><dd>含まれる範囲を確認中</dd></div></dl><p class="small muted">総額・追加費用・お支払い条件は、正式なご案内でお知らせします。再施術には別途費用がかかり、返金保証はありません。</p></div></div></section>
    <section id="risks" class="section risks-section"><div class="wrap"><div class="section-heading"><p class="eyebrow">RISKS & AFTERCARE</p><h2>治療後の過ごし方と、<br>知っておきたいリスク。</h2></div><div class="risk-columns"><div><h3>主なリスク・副作用</h3><ul class="bullet-list">${C.medical.risks.map((r) => `<li>${escape(r)}</li>`).join("")}</ul></div><div><h3>施術後の経過・生活の目安</h3><dl class="aftercare-list">${C.medical.downtime.map(([term, text]) => `<div><dt>${escape(term)}</dt><dd>${escape(text)}</dd></div>`).join("")}</dl></div></div><div class="risk-note"><h3>診察時にお伝えいただきたいこと</h3><p>${escape(C.medical.precautions)}</p><p>症状や回復の経過には個人差があります。異常な痛み・腫れ・発熱などがある場合は、速やかに医療機関へ連絡してください。</p></div></div></section>
